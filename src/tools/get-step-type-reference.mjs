...
<cc:workday-in id="StartHere" routes-to="BuildRequest">
  <cc:integration-system name="INT999_My_Integration">
    ...
    <!-- RAAS (Report as a Service): declare all custom reports this integration calls.
         ...
         Confirmed from INT999, INT999, INT999, INT999, INT999 — ALL use cloud:report-service.
    ...
    <!-- Retrieval service: accesses files uploaded by the user on the integration event.
         Confirmed from INT999. Pair with vm://wcc/GetEventDocuments to fetch the files.
...
<!-- POST example (INT999 pattern — payload comes directly from XML via xslt-plus) -->
...
<!-- ── Additional attributes (confirmed INT999) ──────────────────────────
...
Please contact HR Ops.&lt;br&gt;</cc:text>
  ...
  'store': {
    ...
    routes_via: 'none (inline step inside cc:async-mediation cc:steps)',
    xml_example: `<!-- ── Store a CSV file to the integration event (INT999 pattern) ─────────
...
<!-- ── Store a document with full Atom metadata (INT999 pattern) ──────────
...
<cc:store id="StoreFromVar"
  ...
  'base64-encode': {
    ...
    routes_via: 'none (inline step inside cc:async-mediation cc:steps)',
    xml_example: `<!-- ── Basic Auth header from API key (INT999 Greenhouse pattern) ──────────
...
<cc:async-mediation id="DoFormatDate" routes-to="Continue">
  ...
  'json-splitter': {
    ...
    xml_example: `<!-- Split raw JSON on $.attachments array (INT999 Greenhouse pattern)
