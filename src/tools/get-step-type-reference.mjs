...
<cc:splitter id="SplitWorkers" no-split-message-error="false">
  ...
  'note': {
    description: 'Documentation-only element. Does no processing. Use as a terminal node in assembly.xml when you need a named end-point for routing (routes-to="End").\n\nCRITICAL DIAGRAM RULE: cc:note MUST NOT appear anywhere in assembly-diagram.xml. Studio\'s Factory.scala has no EditPart case for cc:note — adding it to <visualProperties>, swimlane <elements>, or <connections> crashes Studio with scala.MatchError at every load. Same crash as cc:send-error.\n\nSafe usage: cc:note can exist in assembly.xml as a routing terminal, but must be completely absent from assembly-diagram.xml. The diagram simply has no node for it — connections route to the last rendered step instead.',
    routes_via: 'none (terminal)',
    xml_example: `<!-- assembly.xml: safe as a routing terminal -->
<cc:note id="End">
  <cc:description>Integration complete. No further processing.</cc:description>
</cc:note>

<!-- assembly-diagram.xml: cc:note is ABSENT — do not add it here.
     Any route pointing to "End" in the assembly terminates without a diagram node. -->`,
  },
  'eval': {
